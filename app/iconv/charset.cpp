...
#define  detact_debug(fmt, args...) { if (___debug_mode) { \
    printf(fmt, ##args); \
} }
...
bool radar::detact(const char *data, int len, acl::string &result_charset)
{
    typedef struct {
    ...
    for (csp = eas_cs; csp->group_id; csp++)
    {
        ...
        else if (csp->result_len == max_len)
        {
            if (csp_find)
            {
                if (csp->group_id != csp_find->group_id)
                {
    ...
    for (csp = eas_cs; csp->group_id; csp++)
    {
        ...
        w_count = ___chinese_word_count(data2.c_str(), data2.length());
...
bool radar::detact(acl::string &data, acl::string &result_charset)
{
